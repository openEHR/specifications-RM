...
* _directory_: reference to first folder structure, for compatibility with pre-Release 1.1.0 systems;
...
The `_directory_` reference may be considered _by convention_ as referring to a folder hierarchy that is _shared by all users of the EHR_ to which it is attached, e.g. all medical specialities, departments or settings. It might thus be used to represent episodes. In this case, the other Folder hierarchies referred to by `_folders_` may be created, maintained and used by different departments, specialities etc. However, completely different usage of the folder structures is possible. Use of Folder archetypes should be considered for communicating the intended usage in a particular system.
