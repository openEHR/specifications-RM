...
The `EHR_STATUS._is_modifiable_` attribute is used to indicate whether _the content of an EHR_ is modifiable, and is set to `True` if the EHR is considered _active_. An EHR's 'content' consists of the data within its Compositions (i.e. `_compositions_` attribute) and Directory (i.e. `_directory_` attribute). An active EHR is used within a given EHR system as the primary patient record for a subject. An EHR may be deactivated in the following circumstances:
...
In any of these situations, the EHR may be deactivated by setting `_is_modifiable_` to `False`. An inactive EHR is still visible in the system, and will remain queryable, unless the `_is_queryable_` attribute is also set to `False`.
