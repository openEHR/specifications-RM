...
Note: Some messaging standards make use of a "time validity" or similar attribute on identifier datatypes. 
The recommended way of representing validity timespans of identifiers in openEHR is to model this using archetypes
and templates so that regional or use-case specific needs can be better represented.
...
In addition, a type of item being identified can also be recorded, such as "driver's licence" or "Medicare card". All fields are text fields, rather than coded, as no definitive vocabularies are available. However, some useful sources of terms exists, such as https://www.hl7.org/fhir/v2/0203/index.html[HL7v2 Table 203] for the `type` field. If using the latter, it is recommended to use the 'description' not the code.
...
include::{uml_export_dir}/classes/data_value.adoc[]

include::{uml_export_dir}/classes/dv_boolean.adoc[]

include::{uml_export_dir}/classes/dv_state.adoc[]

include::{uml_export_dir}/classes/dv_identifier.adoc[]
