...
The syntax of a `DV_EHR_URI` is an openEHR path, inside the 'ehr' URI scheme-space, and is of the
form (Antlr parser rule):

----
ehr://system_id/ehr_id/top_level_structure_locator/path_inside_top_level_structure
----
:overview_ref: http://www.openehr.org/releases/BASE/{release}/architecture_overview.html#_ehr_uris

The syntax is described in more detail in the {overview_ref}[openEHR Architecture Overview]. `DV_EHR_URIs` are used as a mechanism for referencing in the EHR, ensuring readability
by humans, as well as validity when extracts are transmitted elsewhere: even if the target of a path is
not present, the path can be used to locate the missing item on demand.
