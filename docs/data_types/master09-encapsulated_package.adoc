...
image::{uml_export_dir}/diagrams/RM-data_types.encapsulated.svg[id=rm_data_types_encapsulated, align="center"]
...
|inline JPG with +
 SHA-2 hash             |`DV_MULTIMEDIA`    |`_data_` = byte array carrying JPG +
                                             ...
                                             `_integrity_check_algorithm_ = "SHA-2"` +
                                             `_size_` = Integer indicating number of bytes of `_data_`.                                             
