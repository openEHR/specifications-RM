...
|[[latest_issue]]
|{spec_tickets}/SPECPR-386[SPECPR-386^]: Clarify that `DV_DURATION._magnitude()_` is computed via `Iso8601_duration._to_seconds()_`, and add a cross-reference link from `_magnitude()_` to the `Iso8601_duration` class in the BASE Foundation Types specification.
|P Pazos, +
 S Iancu
