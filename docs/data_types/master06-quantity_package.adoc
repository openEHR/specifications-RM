...
the reflex response to tendon percussion as pass:["-", "+/-", "+", "++", "+++", "++++"]. There may be no
way to scientifically precisely quantify such values because they reflect a subjective experience of the
patient or informal judgement by clinician. However, they are understood as being ordered, e.g. pass:["++"]
is 'greater than' pass:["+"].
...
A second requirement for ordinal values is that in many cases there is a need to associate integer values
...
Some countable entities such as tablets are divisible into major fractions, typically halves and occasionally
quarters.
...
In general ratios have real number values, even if many examples appear to be integer ratios. Proportions
with unitary denominator and % (denominator = 100) are common.
...
Although a single solute/single solvent formulation appears to have the same form as a ratio, the general
...
has a therapeutic range of 1.0 - 2.5 mmol/L and a toxic range > 3.6 mmol/L
...
Where there are multiple ranges, the important question is: _which range information is relevant to the
actual data being recorded for the patient?_ In theory, only the range corresponding to the particular
...
It is quite common for laboratories to include a normal range with each measured value, and/or a normal
...
high), L, LL, LLL in HL7v2 messaging, although other schemes are undoubtedly used.
...
which use symbolic renderings instead, such as pass:["+", "++", "+++"], or "mild", "medium", "severe".
...
`_add_`, `_subtract_` and `_diff_` are defined rather than '+' or '-'. Date/time types, as well as the relative
...
defined by the {ucum}[Unified Code for Units of Measure (UCUM)]. A valid units string always implies a
measured property, such as "force" or "pressure". The property of a Quantity can conveniently be constrained in archetypes, e.g. to "pressure", which would allow any pressure unit. Unit strings can be
...
NOTE: while these semantics will allow comparison of e.g. two pressures
...
`_accuracy_unknown_` is represented by a Void (i.e. null) value for the accuracy attribute. An abstract
Boolean feature `_accuracy_unknown_` is defined in the parent class `DV_QUANTIFIED` to provide a logical
...
Lastly, the `_is_integral_` function indicates that the numerator and denominator are both integer values;
...
included via the attribute `DV_ORDERED`.`_normal_range_`, of type `REFERENCE_RANGE`. Other reference
ranges (e.g. sub-critical, critical etc) can be included via the attribute
`DV_ORDERED`.`_other_reference_ranges_`. The separation of normal and other reference range attributes
is used because the former constitute the vast majority of ranges quoted for quantitative data.

Normal status can be included via the attribute `DV_ORDERED`.`_normal_status_`, which takes the form of
...
=== Units Syntax
The BNF syntax specification of the units string, adapted from {ucum}[UCUM] is as follows:  
...
This proposal is comprehensive, covering all useful unit systems, including SI, various imperial, customary
