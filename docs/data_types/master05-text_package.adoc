...
* `"commonmark"`: the text is assumed to be formatted using the {commonmark_spec}[CommonMark] form of markdown, and should preferably be rendered via conversion to HTML, but _may_ be passed straight through to display if unavoidable, since markdown is acceptably human-readable;
...
[.deprecated]
*Deprecated*: Starting at RM Release 1.0.4, the use of the `_hyperlink_` field is deprecated in favour of inline link(s) in the `_value_` field, represented using CommonMark markup, whose most basic form is `[text](uri)`.

[.tbd]
TBD: what subset if any do we allow of CommonMark? Do we want to allow HTML blocks or raw HTML, which are both possible in CommonMark? What about images?

[.tbd]
TBD: do we state a minimum version of CommonMark?
