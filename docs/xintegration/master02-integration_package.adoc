...
described here is data expressed in a form of a CEN EN13606 Extract. Part 1 of EN13606 defines a
information model which is nearly identical to that of openEHR at the `COMPOSITION` and `SECTION`
levels. The CEN EN13606 `Entry` class is a generic structure with a minimum of contextual metadata,
...
image::{uml_export_dir}/diagrams/RM-integration.svg[id=integration_package, align="center"]
...
=== Integration with CEN EN13606

The `GENERIC_ENTRY` class provides a convenient basis for making openEHR systems EN13606-
compliant, which in turn gives openEHR a gateway capability in heterogeneous environments where
EN13606 is being used to communicate data. A CEN EN13606 EHR Extract can be converted to a
...
intermediate form for further conversion into EN13606 EHR Extracts.
...
image::diagrams/integration_environment.png[id=integration_environment, align="center", width=80%]
...
include::{uml_export_dir}/classes/generic_entry.adoc[]
