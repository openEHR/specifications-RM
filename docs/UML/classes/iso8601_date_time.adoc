...
h|
|*as_string*: `String`
a|ISO8601 string for date/time, in format YYYYMMDDThhmmss[(,&#124;.)s[s[s[s[s[s]]]]]][Z&#124;±hh[mm]] or in extended format YYYY-MM-DDThh:mm:ss[(,&#124;.)s[s[s[s[s[s]]]]]][Z&#124;±hh[mm]] or a partial variant; see valid_iso8601_date_time() below.

h|
|*valid_iso8601_date_time* (s: `String`): `Boolean`
a|String is a valid ISO 8601 date-time, i.e. takes the form:

* YYYYMMDDThhmmss[(,&#124;.)s[s[s[s[s[s]]]]]][Z&#124;±hh[mm]]
