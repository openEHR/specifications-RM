...
The version identification scheme described here is adapted from the work of Hnìtynka and Plášil <<Hnìtynka_2004>>. `VERSION` objects are identified by a `_uid_` attribute, which is a three-part identifier consisting of the attributes `_object_id_`, `_creating_system_id_` and `_version_tree_id_` (see   {openehr_rm_support}#_identification_package[`support.identification' package in the Support IM]), in which the `_object_id_` part is a copy of the `_uid_` of the owning `VERSIONED_OBJECT` version container.
