...
An Original version may also be marked as being in the `inactive` state. This allows content that is understood as informationally valid with respect to the real world activities it represents - regardless of completion state - to be marked as no longer valid for use. The exact meaning of 'valid' will depend on the content: it may be that the content is no longer relevant or useful with respect to its original purpose. An `inactive` item be be reactivated back to the `complete` state, or retrieved back to the `incomplete` state (see table below for examples of when). 
...
|`inactive`     |`retrieve`     |`incomplete`   |A de-activated care plan is used as the draft for a new care plan.
