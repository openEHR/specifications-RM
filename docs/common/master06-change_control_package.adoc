...
Content in Original versions has a lifecycle state associated with it, modelled using the `ORIGINAL_VERSION._lifecycle_state_` attribute, which is coded from the openEHR Terminology 'version lifecycle state' group. The possible values are `532|complete|`, `553|incomplete|`, `523|deleted|`, and `680|inactive|`. 
...
The transitions related to the `inactive` state are as follows.
...
|`incomplete`   |`suspend`      |`inactive`     |A new care plan (the version content) was made in rehabilitation centre, but patient transferred to hospital before care plan was completed. 
                                                 Should be archived for later reference e.g. if the patient comes to rehab again.

|`inactive`     |`resume`       |`incomplete`   |Patient returns to rehab and care is resumed; the care plan is therefore brought back into use.
