...
The version identification scheme described here is adapted from the work of Hnìtynka and Plášil <<Hnìtynka_2004>>. `VERSION` objects are identified by a uid attribute, which is a three-part identifier consisting of the attributes `_object_id_`, `_creating_system_id_` and `_version_tree_id_` (see `support.identification` package in the Common IM). The first part of the `VERSION` identifier - the `_object_id_` attribute - is a copy of the `_uid_` of the `VERSIONED_OBJECT` container in which the `VERSION` was originally created. The second and third parts of the identifier are explained below. The figure below illustrates the scheme graphically.
