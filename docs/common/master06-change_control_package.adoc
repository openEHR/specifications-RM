...
The version identification scheme described here is adapted from the work of Hnìtynka and Plášil <<Hnìtynka_2004>>. `VERSION` objects are identified by a `_uid_` attribute, which is a three-part identifier consisting of the attributes `_object_id_`, `_version_tree_id_` and `_creating_system_id_` (see `support.identification` package in the Support IM <<openehr_rm_support>>). The first part of the `VERSION` identifier - the `_object_id_` attribute - is a copy of the `_uid_` of the `VERSIONED_OBJECT` container in which the `VERSION` was originally created. The second and third parts of the identifier are explained below. The figure below illustrates the scheme graphically.
