...
The important semantics of the Task concept in the model are specified in the `TASK_PLAN_ITEM` class, with further specifics appearing in the `TASK_GROUP` and `TASK` descendants. Key attributes of a `TASK_PLAN_ITEM` include:
...
The model described here thus makes no _a priori_ assumptions about granularity of representation of a given real-world task, and both representations are possible. This first kind is thus represented as an instance of the class `DEFINED_TASK`, while the second is achieved with the use of an `EXTERNAL_TASK` which refers out to another `TASK_PLAN` to be executed by the same performer.

The `EXTERNAL_TASK` type gives rise to the possibility of a hierarchy of linked Task Plans, with each successive member in the chain representing a more detailed form of a given real world task. This can be visualised as follows.

[.text-center]
.Task Plan linking
image::diagrams/task_linking.svg[id=task_linking, align="center", width=70%]

A structure like the above may be used differently by performers of differing training levels - since each `EXTERNAL_TASK` has its own `_description_`, an expert user may require no further list of steps, but a trainee might make use of the full hierarchical structure. The `_training_level_` attribute on `TASK_PLAN` can be used to control this. A higher value corresponds to a lower level of expertise, and different values can be set on different linked `TASK_PLANs`. The workflow application may treat different users as being of different training levels, in which case Task Plans of higher training levels can be hidden. In the above example, a principal performer with trainee level = 7 will see the three Task Plans on the left, but not the final one, since it has a trainee level of 10.
...
One reason to allow a Task to contain a `_prototype_` reference that remains unresolved until runtime is if the Task represents the act of making an observation, for example, taking a blood pressure. In such cases, no prototype at all may be needed, and the Task `_description_` attribute (inherited from `TASK_PLAN_ITEM`) may be sufficient information for the performer. On the other hand, a prototype `OBSERVATION` could be specified in the `TASK_PLAN` template, which defines a particular form of the observation, e.g. a blood pressure which only records mean arterial pressure and cuff size.
...
IN addition to timing, other conditions may determine when or if a Task can be performed. These are modelled as the `TASK_PLAN_ITEM` attribute `_preconditions_: TASK_PRECONDITION`. The precondition is formally represented as a `DV_PARSABLE`, which would normally contain a string of {openehr_expression}[openEHR Expression Language] syntax, but may contain another syntax.
...
A more advanced application of Task Plans includes decision pathways, consisting of a decision point and a number of dependent pathays. This is used to represent the decision pathways found in guidelines.
...
include::{uml_export_dir}/classes/task_plan_item.adoc[]
