...
The `CONTRIBUTION` class also contains an audit attribute. Whenever a `CONTRIBUTION` is committed, this attribute captures to the time, place and committer of the committal act; these three attributes (`_system_id_`, `_committer_`, `_time_committed_` of `AUDIT_DETAILS`) should be copied into the corresponding attributes of the `_commit_audit_` of each `VERSION` included in the `CONTRIBUTION`. This is done to enable sharing of versioned entities independently of which Contributions they were part of.
