...
image::ROOT:uml/RM-ehr_extract.openehr_extract.svg[id=rm_openehr_extract, align="center"]
...
cases, and is the structure shown in <<_typical_extract_structure>> above.

=== Demographic Referencing

Within the content an EHR Extract, wherever `PARTICIPATION` and `PARTY_PROXY` structures occur
(for the latter, the `PARTY_SEL`F and `PARTY_IDENTIFIED` descendants), they are preserved intact,
...
structure is illustrated at the bottom of <<_typical_extract_structure>>. In this figure, the `OBJECT_ID`.`_value_` has been
rewritten to refer to the uid (11111111) of the first `OPENEHR_CONTENT_ITEM` object shown in the figure.

== Class Descriptions

include::ROOT:partial$classes/openehr_content_item.adoc[]

include::ROOT:partial$classes/x_versioned_object.adoc[]

include::ROOT:partial$classes/x_versioned_ehr_access.adoc[]

include::ROOT:partial$classes/x_versioned_ehr_status.adoc[]

include::ROOT:partial$classes/x_versioned_composition.adoc[]

include::ROOT:partial$classes/x_versioned_folder.adoc[]

include::ROOT:partial$classes/x_versioned_party.adoc[]
