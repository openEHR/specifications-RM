...
such as an instance of the `ELEMENT` class from the EHR RM. There is no explicit model of negation in the openEHR Reference Model. Negation statements are normally solved by specific 'exclusion' archetypes.
...
WARNING: Additionally, the `_value_` field was assumed to contain no newlines. This assumption is no longer made starting from RM Release 1.0.4, since `DV_PARAGRAPH` is no longer used to represent paragraphs.
