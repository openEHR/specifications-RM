...
2+|Inherits: xref:{class_any}[Any], xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_data_structure}[DATA_STRUCTURE]
...
|*summary*: `xref:{class_item_structure}[ITEM_STRUCTURE]` [0..1]
...
|*events*: `xref:{class_list}[List]<xref:{class_event}[EVENT]<T>>` [0..1]
...
|*as_hierarchy* (): `xref:{class_item}[ITEM]` [1..1]
a|Hierarchical equivalent of the physical representation of each subtype, compatible with CEN EN 13606 structures. +
_Inherited from xref:{class_data_structure}[DATA_STRUCTURE]_

|*is_periodic* (): `xref:{class_boolean}[Boolean]` [1..1]
