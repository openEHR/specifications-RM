...
2+|Inherits: xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_data_structure}[DATA_STRUCTURE]
...
|*events*: `link:/classes/List[List] <xref:{class_event}[EVENT]<T>>` [0..1]
