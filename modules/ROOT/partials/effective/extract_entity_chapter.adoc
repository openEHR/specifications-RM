...
2+|Inherits: xref:{class_any}[Any], xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_extract_chapter}[EXTRACT_CHAPTER]
...
|*items*: `xref:{class_list}[List]<xref:{class_extract_item}[EXTRACT_ITEM]>` [0..1]
a|The information content of this chapter. +
_Inherited from xref:{class_extract_chapter}[EXTRACT_CHAPTER]_

|*extract_id_key*: `xref:{class_string}[String]` [1..1]
