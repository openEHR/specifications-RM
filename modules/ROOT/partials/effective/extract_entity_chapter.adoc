...
2+|Inherits: xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_extract_chapter}[EXTRACT_CHAPTER]
...
|*items*: `link:/classes/List[List] <xref:{class_extract_item}[EXTRACT_ITEM]>` [0..1]
