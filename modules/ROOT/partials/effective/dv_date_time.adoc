...
2+|Inherits: xref:{class_basic_definitions}[BASIC_DEFINITIONS], xref:{class_openehr_definitions}[OPENEHR_DEFINITIONS], xref:{class_any}[Any], xref:{class_data_value}[DATA_VALUE], xref:{class_ordered}[Ordered], xref:{class_dv_ordered}[DV_ORDERED], xref:{class_dv_quantified}[DV_QUANTIFIED], xref:{class_dv_absolute_quantity}[DV_ABSOLUTE_QUANTITY], xref:{class_any}[Any], xref:{class_ordered}[Ordered], xref:{class_temporal}[Temporal], xref:{class_time_definitions}[Time_Definitions], xref:{class_iso8601_type}[Iso8601_type], xref:{class_dv_temporal}[DV_TEMPORAL], xref:{class_iso8601_date_time}[Iso8601_date_time]
...
|*accuracy*: `xref:{class_dv_duration}[DV_DURATION]` [0..1]
a|Time accuracy, expressed as a duration. +
_Inherited from xref:{class_dv_temporal}[DV_TEMPORAL]_
...
|*magnitude* (): `xref:{class_double}[Double]` [1..1]
...
): `xref:{class_dv_date_time}[DV_DATE_TIME]` [1..1]
...
|*diff* __alias__ "-" ( +
other: `xref:{class_dv_date_time}[DV_DATE_TIME][1]` +
): `xref:{class_dv_duration}[DV_DURATION]` [1..1]
...
_Inherited from xref:{class_iso8601_date_time}[Iso8601_date_time]_
