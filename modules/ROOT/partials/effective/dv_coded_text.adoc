...
2+|Inherits: xref:{class_basic_definitions}[BASIC_DEFINITIONS], xref:{class_openehr_definitions}[OPENEHR_DEFINITIONS], xref:{class_data_value}[DATA_VALUE], xref:{class_dv_text}[DV_TEXT]
...
_Inherited from xref:{class_dv_text}[DV_TEXT]_

|*hyperlink*: `xref:{class_dv_uri}[DV_URI]` [0..1]
...
|*formatting*: `xref:{class_string}[String]` [0..1]
...
|*mappings*: `xref:{class_list}[List]<xref:{class_term_mapping}[TERM_MAPPING]>` [0..1]
...
|*encoding*: `xref:{class_code_phrase}[CODE_PHRASE]` [0..1]
...
|*defining_code*: `xref:{class_code_phrase}[CODE_PHRASE]` [1..1]
