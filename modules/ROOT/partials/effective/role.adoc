[cols="2,3", frame=none, grid=rows]
|===
2+^h|*ROLE*

2+a|Generic description of a role performed by an Actor. The role corresponds to a competency of the Party. Roles are used to define the responsibilities undertaken by a Party for a purpose. Roles should have credentials qualifying the performer to perform the role.

2+|Inherits: xref:{class_any}[Any], xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_party}[PARTY]

2+h|*Attributes*

|*name*: `xref:{class_dv_text}[DV_TEXT]` [1..1]
a|Runtime name of this fragment, used to build runtime paths. This is the term provided via a clinical application or batch process to name this EHR construct: its retention in the EHR faithfully preserves the original label by which this entry was known to end users. +
_Inherited from xref:{class_locatable}[LOCATABLE]_

|*archetype_node_id*: `xref:{class_string}[String]` [1..1]
a|Design-time archetype identifier of this node taken from its generating archetype; used to build archetype paths. Always in the form of an at-code, e.g.  `at0005`. This value enables a 'standardised' name for this node to be generated, by referring to the generating archetype local terminology. 

At an archetype root point, the value of this attribute is always the stringified form of the `_archetype_id_` found in the `_archetype_details_` object. +
_Inherited from xref:{class_locatable}[LOCATABLE]_

|*uid*: `xref:{class_uid_based_id}[UID_BASED_ID]` [0..1]
a|Optional globally unique object identifier for root points of archetyped structures. +
_Inherited from xref:{class_locatable}[LOCATABLE]_

|*links*: `xref:{class_list}[List]<xref:{class_link}[LINK]>` [0..1]
a|Links to other archetyped structures (data whose root object inherits from `ARCHETYPED`, such as `ENTRY`, `SECTION` and so on). Links may be to structures in other compositions. +
_Inherited from xref:{class_locatable}[LOCATABLE]_

|*archetype_details*: `xref:{class_archetyped}[ARCHETYPED]` [0..1]
a|Details of archetyping used on this node. +
_Inherited from xref:{class_locatable}[LOCATABLE]_

|*feeder_audit*: `xref:{class_feeder_audit}[FEEDER_AUDIT]` [0..1]
a|Audit trail from non-openEHR system of original commit of information forming the content of this node, or from a conversion gateway which has synthesised this node. +
_Inherited from xref:{class_locatable}[LOCATABLE]_

|*identities*: `xref:{class_list}[List]<xref:{class_party_identity}[PARTY_IDENTITY]>` [1..1]
a|Identities used by the party to identify itself, such as legal name, stage names, aliases, nicknames and so on. +
_Inherited from xref:{class_party}[PARTY]_

|*contacts*: `xref:{class_list}[List]<xref:{class_contact}[CONTACT]>` [0..1]
a|Contacts for this party. +
_Inherited from xref:{class_party}[PARTY]_

|*details*: `xref:{class_item_structure}[ITEM_STRUCTURE]` [0..1]
a|All other details for this Party. +
_Inherited from xref:{class_party}[PARTY]_

|*reverse_relationships*: `xref:{class_list}[List]<xref:{class_locatable_ref}[LOCATABLE_REF]>` [0..1]
a|References to relationships in which this Party takes part as target. +
_Inherited from xref:{class_party}[PARTY]_

|*relationships*: `xref:{class_list}[List]<xref:{class_party_relationship}[PARTY_RELATIONSHIP]>` [0..1]
a|Relationships in which this Party takes part as source. +
_Inherited from xref:{class_party}[PARTY]_

|*time_validity*: `xref:{class_dv_interval}[DV_INTERVAL]<xref:{class_dv_date}[DV_DATE]>` [0..1]
a|Valid time interval for this role.

|*performer*: `xref:{class_party_ref}[PARTY_REF]` [1..1]
a|Reference to Version container of Actor playing the role.

|*capabilities*: `xref:{class_list}[List]<xref:{class_capability}[CAPABILITY]>` [0..1]
a|The capabilities of this role.

2+h|*Functions*

|_(abstract)_ *is_equal* ( +
other: `xref:{class_any}[Any][1]` +
): `xref:{class_boolean}[Boolean]` [1..1]
a|Value equality: return True if `this` and `_other_` are attached to objects considered to be equal in value. +
_Inherited from xref:{class_any}[Any]_

.Parameters +
[horizontal]
`_other_`:: Other object for comparison.

|*equal* __alias__ "=", "==" ( +
other: `xref:{class_any}[Any][1]` +
): `xref:{class_boolean}[Boolean]` [1..1]
a|Reference equality for reference types, value equality for value types. +
_Inherited from xref:{class_any}[Any]_

.Parameters +
[horizontal]
`_other_`:: Other object for comparison.

|*instance_of* ( +
a_type: `xref:{class_string}[String][1]` +
): `xref:{class_any}[Any]` [1..1]
a|Create new instance of a type. +
_Inherited from xref:{class_any}[Any]_

|*type_of* ( +
an_object: `xref:{class_any}[Any][1]` +
): `xref:{class_string}[String]` [1..1]
a|Type name of an object as a string. May include generic parameters, as in `"Interval<Time>"`. +
_Inherited from xref:{class_any}[Any]_

|*not_equal* __alias__ "!=", "≠" ( +
other: `xref:{class_ordered}[Ordered][1]` +
): `xref:{class_boolean}[Boolean]` [1..1]
a|True if current object not equal to `_other_`. Returns not `_equal_()`. +
_Inherited from xref:{class_any}[Any]_

|*parent* (): `xref:{class_pathable}[PATHABLE]` [1..1]
a|Parent of this node in a compositional hierarchy. +
_Inherited from xref:{class_pathable}[PATHABLE]_

|*item_at_path* ( +
a_path: `xref:{class_string}[String][1]` +
): `xref:{class_any}[Any]` +
 +
__Pre__: `path_unique (a_path)` [1..1]
a|The item at a path (relative to this item); only valid for unique paths, i.e. paths that resolve to a single item. +
_Inherited from xref:{class_pathable}[PATHABLE]_

|*items_at_path* ( +
a_path: `xref:{class_string}[String][1]` +
): `xref:{class_list}[List]<xref:{class_any}[Any]>` [0..1]
a|List of items corresponding to a non-unique path. +
_Inherited from xref:{class_pathable}[PATHABLE]_

|*path_exists* ( +
a_path: `xref:{class_string}[String][1]` +
): `xref:{class_boolean}[Boolean]` +
 +
__Pre__: `not a_path.is_empty` [1..1]
a|True if the path exists in the data with respect to the current item. +
_Inherited from xref:{class_pathable}[PATHABLE]_

|*path_unique* ( +
a_path: `xref:{class_string}[String][1]` +
): `xref:{class_boolean}[Boolean]` +
 +
__Pre__: `path_exists (a_path)` [1..1]
a|True if the path corresponds to a single item in the data. +
_Inherited from xref:{class_pathable}[PATHABLE]_

|*path_of_item* ( +
a_loc: `xref:{class_pathable}[PATHABLE][1]` +
): `xref:{class_string}[String]` [1..1]
a|The path to an item relative to the root of this archetyped structure. +
_Inherited from xref:{class_pathable}[PATHABLE]_

|*concept* (): `xref:{class_dv_text}[DV_TEXT]` [1..1]
a|Clinical concept of the archetype as a whole (= derived from the archetype_node_id' of the root node) +
_Inherited from xref:{class_locatable}[LOCATABLE]_

|*is_archetype_root* (): `xref:{class_boolean}[Boolean]` [1..1]
a|True if this node is the root of an archetyped structure. +
_Inherited from xref:{class_locatable}[LOCATABLE]_

|*type* (): `xref:{class_dv_text}[DV_TEXT]` [1..1]
a|Type of party, such as  `PERSON`,  `ORGANISATION`, etc. Role name, e.g.  general practitioner ,  nurse ,  private citizen . Taken from inherited `_name_` attribute. +
_Inherited from xref:{class_party}[PARTY]_

2+h|*Invariants*

2+a|__Links_valid__: `links /= Void implies not links.is_empty` +
_Inherited from xref:{class_locatable}[LOCATABLE]_

2+a|__Archetyped_valid__: `is_archetype_root xor archetype_details = Void` +
_Inherited from xref:{class_locatable}[LOCATABLE]_

2+a|__Archetype_node_id_valid__: `not archetype_node_id.is_empty` +
_Inherited from xref:{class_locatable}[LOCATABLE]_

2+a|__Type_valid__: `type = name` +
_Inherited from xref:{class_party}[PARTY]_

2+a|__Identities_valid__: `not identities.is_empty` +
_Inherited from xref:{class_party}[PARTY]_

2+a|__Contacts_valid__: `contacts /= Void implies not contacts.is_empty` +
_Inherited from xref:{class_party}[PARTY]_

2+a|__Relationships_validity__: `relationships /= Void implies (not relationships.is_empty and then relationships.for_all (r &#124; r.source = self)` +
_Inherited from xref:{class_party}[PARTY]_

2+a|__Reverse_relationships_validity__: `reverse_relationships /= Void implies (not reverse_relationships.empty and then reverse_relationships.for_all (item &#124; repository ("demographics").all_party_relationships.has_object (item) and then repository("demographics").all_party_relationships.object (item).target = self))` +
_Inherited from xref:{class_party}[PARTY]_

2+a|__Is_archetype_root__: `is_archetype_root` +
_Inherited from xref:{class_party}[PARTY]_

2+a|__Uid_mandatory__: `uid /= Void` +
_Inherited from xref:{class_party}[PARTY]_

2+a|__Capabilities_valid__: `capabilities /= Void implies not capabilities.empty`

|===
