...
|*includes_versions*: `xref:{class_boolean}[Boolean]` [1..1]
a|True if the Versions from the Contribution are included; False if just the Contribution and its Audit are included.

|*contribution_list*: `xref:{class_list}[List]<xref:{class_hier_object_id}[HIER_OBJECT_ID]>` [0..1]
...
|*all_contributions*: `xref:{class_boolean}[Boolean]` [0..1]
