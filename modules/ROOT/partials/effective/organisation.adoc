...
2+|Inherits: xref:{class_any}[Any], xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_party}[PARTY], xref:{class_actor}[ACTOR]
...
_Inherited from xref:{class_actor}[ACTOR]_
