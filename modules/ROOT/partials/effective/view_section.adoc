...
2+|Inherits: xref:{class_any}[Any], xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_content_item}[CONTENT_ITEM], xref:{class_view_item}[VIEW_ITEM]
...
|*items*: `xref:{class_list}[List]<xref:{class_view_item}[VIEW_ITEM]>` [0..1]
a|
