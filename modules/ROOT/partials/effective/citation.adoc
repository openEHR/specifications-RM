...
2+|Inherits: xref:{class_any}[Any], xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_content_item}[CONTENT_ITEM], xref:{class_view_item}[VIEW_ITEM], xref:{class_view_entry}[VIEW_ENTRY]
...
|*meta_data*: `xref:{class_cluster}[CLUSTER]` [0..1]
a| +
_Inherited from xref:{class_view_item}[VIEW_ITEM]_

|*source_type*: `xref:{class_string}[String]` [0..1]
a|

|*source_ref*: `xref:{class_locatable_ref}[LOCATABLE_REF]` [1..1]
a|Reference to original object.

|*source_parent_ref*: `xref:{class_locatable_ref}[LOCATABLE_REF]` [1..1]
