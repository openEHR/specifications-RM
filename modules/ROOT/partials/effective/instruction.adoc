...
|*narrative*: `xref:{class_dv_text}[DV_TEXT]` [1..1]
a|Mandatory human-readable version of what the Instruction is about.

|*expiry_time*: `xref:{class_dv_date_time}[DV_DATE_TIME]` [0..1]
...
|*wf_definition*: `xref:{class_dv_parsable}[DV_PARSABLE]` [0..1]
a|Optional workflow engine executable expression of the Instruction.

|*activities*: `xref:{class_list}[List]<xref:{class_activity}[ACTIVITY]>` [0..1]
