...
|*repeat_period*: `xref:{class_dv_duration}[DV_DURATION]` [0..1]
a|Period for resending update Extracts in response to original Request.

|*trigger_events*: `link:/classes/List[List] <xref:{class_dv_coded_text}[DV_CODED_TEXT]>` [0..1]
...
|*update_method*: `xref:{class_code_phrase}[CODE_PHRASE]` [1..1]
