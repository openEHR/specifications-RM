...
2+|Inherits: xref:{class_basic_definitions}[BASIC_DEFINITIONS], xref:{class_openehr_definitions}[OPENEHR_DEFINITIONS], xref:{class_data_value}[DATA_VALUE], link:/classes/Ordered[Ordered] , xref:{class_dv_ordered}[DV_ORDERED], xref:{class_dv_quantified}[DV_QUANTIFIED], xref:{class_dv_absolute_quantity}[DV_ABSOLUTE_QUANTITY]
