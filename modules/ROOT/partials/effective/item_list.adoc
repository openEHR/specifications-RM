...
|*items*: `xref:{class_list}[List]<xref:{class_element}[ELEMENT]>` [0..1]
...
|*item_count* (): `xref:{class_integer}[Integer]` [1..1]
a|Count of all items.

|*names* (): `xref:{class_list}[List]<xref:{class_dv_text}[DV_TEXT]>` [0..1]
