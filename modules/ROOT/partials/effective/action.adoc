...
2+|Inherits: xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_content_item}[CONTENT_ITEM], xref:{class_entry}[ENTRY], xref:{class_care_entry}[CARE_ENTRY]
