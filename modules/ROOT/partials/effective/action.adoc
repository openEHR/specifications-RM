...
2+|Inherits: xref:{class_any}[Any], xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_content_item}[CONTENT_ITEM], xref:{class_entry}[ENTRY], xref:{class_care_entry}[CARE_ENTRY]
...
_Inherited from xref:{class_entry}[ENTRY]_
...
|*other_participations*: `xref:{class_list}[List]<xref:{class_participation}[PARTICIPATION]>` [0..1]
...
|*protocol*: `xref:{class_item_structure}[ITEM_STRUCTURE]` [0..1]
...
_Inherited from xref:{class_care_entry}[CARE_ENTRY]_

|*guideline_id*: `xref:{class_object_ref}[OBJECT_REF]` [0..1]
...
|*ism_transition*: `xref:{class_ism_transition}[ISM_TRANSITION]` [1..1]
a|Details of transition in the Instruction state machine caused by this Action.

|*instruction_details*: `xref:{class_instruction_details}[INSTRUCTION_DETAILS]` [0..1]
a|Details of the Instruction that caused this Action to be performed, if there was one.

|*description*: `xref:{class_item_structure}[ITEM_STRUCTURE]` [1..1]
...
|*subject_is_self* (): `xref:{class_boolean}[Boolean]` +
 +
