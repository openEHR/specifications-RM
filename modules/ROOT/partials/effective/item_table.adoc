...
2+|Inherits: xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_data_structure}[DATA_STRUCTURE], xref:{class_item_structure}[ITEM_STRUCTURE]
...
|*rows*: `xref:{class_cluster}[CLUSTER]` [0..1]
...
|*row_names* (): `link:/classes/List[List] <xref:{class_dv_text}[DV_TEXT]>` [0..1]
a|

|*column_names* (): `link:/classes/List[List] <xref:{class_dv_text}[DV_TEXT]>` [0..1]
a|
