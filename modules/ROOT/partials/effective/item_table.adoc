...
2+|Inherits: xref:{class_any}[Any], xref:{class_pathable}[PATHABLE], xref:{class_locatable}[LOCATABLE], xref:{class_data_structure}[DATA_STRUCTURE], xref:{class_item_structure}[ITEM_STRUCTURE]
...
|*rows*: `xref:{class_list}[List]<xref:{class_cluster}[CLUSTER]>` [0..1]
...
|*as_hierarchy* (): `xref:{class_cluster}[CLUSTER]` [1..1]
...
|*row_count* (): `xref:{class_integer}[Integer]` [1..1]
a|Number of rows in the table.

|*column_count* (): `xref:{class_integer}[Integer]` [1..1]
a|Return number of columns in the table.

|*row_names* (): `xref:{class_list}[List]<xref:{class_dv_text}[DV_TEXT]>` [0..1]
a|Return set of row names.

|*column_names* (): `xref:{class_list}[List]<xref:{class_dv_text}[DV_TEXT]>` [0..1]
a|Return set of column names.

|*ith_row* ( +
i: `xref:{class_integer}[Integer][1]` +
): `xref:{class_cluster}[CLUSTER]` [1..1]
a|Return i-th row.
...
): `xref:{class_element}[ELEMENT]` [1..1]
