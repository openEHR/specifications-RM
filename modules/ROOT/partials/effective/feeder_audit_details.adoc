...
|*location*: `xref:{class_party_identified}[PARTY_IDENTIFIED]` [0..1]
...
|*subject*: `xref:{class_party_proxy}[PARTY_PROXY]` [0..1]
a|Identifiers for subject of the received information item.

|*provider*: `xref:{class_party_identified}[PARTY_IDENTIFIED]` [0..1]
a|Optional provider(s) who created, committed, forwarded or otherwise handled the item.

|*time*: `xref:{class_dv_date_time}[DV_DATE_TIME]` [0..1]
