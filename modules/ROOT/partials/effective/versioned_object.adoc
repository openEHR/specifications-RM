...
|*all_version_ids* (): `link:/classes/List[List] <xref:{class_object_version_id}[OBJECT_VERSION_ID]>` [1..1]
a|Return a list of ids of all versions in this object.

|*all_versions* (): `link:/classes/List[List] <xref:{class_version}[VERSION]<T>>` [1..1]
...
a_lifecycle_state: `xref:{class_dv_coded_text}[DV_CODED_TEXT][1]`, +
a_data: `T[1]`, +
an_other_input_uids: `link:/classes/List[List] <xref:{class_object_version_id}[OBJECT_VERSION_ID]>[1]`, +
signing_key: `link:/classes/String[String] [1]` +
): `void` +
 +
__Pre__: `all_version_ids.has (a_preceding_version_uid) or else version_count = 0` [1..1]
