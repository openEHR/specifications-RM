...
h|*Inherit*
2+|`xref:{class_dv_time_specification}[DV_TIME_SPECIFICATION]`
...
h|*1..1 +
(effected)*
|*calendar_alignment* (): `xref:{class_string}[String]`
a|Calendar alignment extracted from value.

h|*1..1 +
(effected)*
|*event_alignment* (): `xref:{class_string}[String]`
a|Event alignment extracted from value.

h|*1..1 +
(effected)*
|*institution_specified* (): `xref:{class_boolean}[Boolean]`
