...
h|*1..1*
|*sender*: `xref:{class_string}[String]`
a|Party sending the message.

h|*1..1*
|*sender_reference*: `xref:{class_string}[String]`
...
h|*1..1*
|*addressees*: `xref:{class_list}[List]<xref:{class_string}[String]>`
a|Intended recipients, in the form of internet addresses.

h|*0..1*
|*urgency*: `xref:{class_integer}[Integer]`
...
h|*1..1*
|*message*: `xref:{class_message}[MESSAGE]`
