...
h|*0..1*
|*version_spec*: `xref:{class_extract_version_spec}[EXTRACT_VERSION_SPEC]`
...
h|*1..1*
|*manifest*: `xref:{class_extract_manifest}[EXTRACT_MANIFEST]`
a|Specification of entities (e.g. records) to include in the Extract.

h|*1..1*
|*extract_type*: `xref:{class_dv_coded_text}[DV_CODED_TEXT]`
...
h|*1..1*
|*link_depth*: `xref:{class_integer}[Integer]`
...
h|*0..1*
|*criteria*: `xref:{class_list}[List]<xref:{class_dv_parsable}[DV_PARSABLE]>`
