...
h|*1..1*
|*current_state*: `xref:{class_dv_coded_text}[DV_CODED_TEXT]`
a|The ISM current state. Coded by openEHR terminology group Instruction states.

h|*0..1*
|*transition*: `xref:{class_dv_coded_text}[DV_CODED_TEXT]`
...
h|*0..1*
|*careflow_step*: `xref:{class_dv_coded_text}[DV_CODED_TEXT]`
...
h|*0..1*
|*reason*: `xref:{class_list}[List]<xref:{class_dv_text}[DV_TEXT]>`
