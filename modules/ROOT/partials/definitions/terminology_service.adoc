...
h|*Inherit*
2+|`xref:{class_openehr_terminology_group_identifiers}[OPENEHR_TERMINOLOGY_GROUP_IDENTIFIERS]`, `xref:{class_openehr_code_set_identifiers}[OPENEHR_CODE_SET_IDENTIFIERS]`
...
h|*0..1*
|*terminology_identifiers* (): `xref:{class_list}[List]<xref:{class_string}[String]>`
...
h|*1..1*
|*openehr_code_sets* (): `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_string}[String]>`
a|Set of all code set identifiers known in the terminology service.

h|*0..1*
|*code_set_identifiers* (): `xref:{class_list}[List]<xref:{class_string}[String]>`
