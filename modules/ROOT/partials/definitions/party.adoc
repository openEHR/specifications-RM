...
h|*1..1*
|*identities*: `xref:{class_list}[List]<xref:{class_party_identity}[PARTY_IDENTITY]>`
a|Identities used by the party to identify itself, such as legal name, stage names, aliases, nicknames and so on.

h|*0..1*
|*contacts*: `xref:{class_list}[List]<xref:{class_contact}[CONTACT]>`
...
h|*0..1*
|*reverse_relationships*: `xref:{class_list}[List]<xref:{class_locatable_ref}[LOCATABLE_REF]>`
a|References to relationships in which this Party takes part as target.

h|*0..1*
|*relationships*: `xref:{class_list}[List]<xref:{class_party_relationship}[PARTY_RELATIONSHIP]>`
