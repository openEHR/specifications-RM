...
h|*1..1*
|*identities*: `link:/classes/List[List] <xref:{class_party_identity}[PARTY_IDENTITY]>`
a|Identities used by the party to identify itself, such as legal name, stage names, aliases, nicknames and so on.

h|*0..1*
|*contacts*: `link:/classes/List[List] <xref:{class_contact}[CONTACT]>`
...
h|*0..1*
|*reverse_relationships*: `link:/classes/List[List] <xref:{class_locatable_ref}[LOCATABLE_REF]>`
...
h|*0..1*
|*relationships*: `link:/classes/List[List] <xref:{class_party_relationship}[PARTY_RELATIONSHIP]>`
...
h|*1..1*
|*type* (): `xref:{class_item_structure}[ITEM_STRUCTURE]`
