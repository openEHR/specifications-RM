...
h|*1..1*
|*include_all_versions*: `xref:{class_boolean}[Boolean]`
a|True if all versions of each item in the Extract are included.

h|*0..1*
|*commit_time_interval*: `xref:{class_dv_interval}[DV_INTERVAL]<xref:{class_dv_date_time}[DV_DATE_TIME]>`
