...
h|*1..1*
|*function*: `xref:{class_dv_text}[DV_TEXT]`
...
h|*1..1*
|*mode*: `xref:{class_dv_coded_text}[DV_CODED_TEXT]`
a|The mode of the performer / activity interaction, e.g. present, by telephone, by email etc.

h|*0..1*
|*time*: `xref:{class_dv_interval}[DV_INTERVAL]<xref:{class_dv_date_time}[DV_DATE_TIME]>`
...
h|*1..1*
|*performer*: `xref:{class_party_proxy}[PARTY_PROXY]`
