...
h|*1..1*
|*narrative*: `xref:{class_dv_text}[DV_TEXT]`
a|Mandatory human-readable version of what the Instruction is about.

h|*0..1*
|*expiry_time*: `xref:{class_dv_date_time}[DV_DATE_TIME]`
...
h|*0..1*
|*wf_definition*: `xref:{class_dv_parsable}[DV_PARSABLE]`
a|Optional workflow engine executable expression of the Instruction.

h|*0..1*
|*activities*: `link:/classes/List[List] <xref:{class_activity}[ACTIVITY]>`
