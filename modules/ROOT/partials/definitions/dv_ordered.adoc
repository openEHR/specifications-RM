...
h|*Inherit*
2+|`xref:{class_data_value}[DATA_VALUE]`, `xref:{class_ordered}[Ordered]`
...
h|*0..1*
|*normal_status*: `xref:{class_code_phrase}[CODE_PHRASE]`
...
h|*0..1*
|*normal_range*: `xref:{class_dv_interval}[DV_INTERVAL]`
a|Optional normal range.

h|*0..1*
|*other_reference_ranges*: `xref:{class_list}[List]<xref:{class_reference_range}[REFERENCE_RANGE]<xref:{class_dv_ordered}[DV_ORDERED]>>`
...
h|*1..1*
|*is_simple* (): `xref:{class_boolean}[Boolean]`
