...
h|*1..1*
|*languages*: `link:/classes/List[List] <xref:RM:data_types:text_package.adoc#_dv_text_class[DV_TEXT]>`
a|Languages which can be used to communicate with this actor, in preferred order of use (if known, else order irrelevant).

h|*1..1*
|*roles*: `link:/classes/List[List] <xref:RM:support:identification_package.adoc#_party_ref_class[PARTY_REF]>`
