...
h|*1..1*
|*time_committed*: `xref:{class_dv_date_time}[DV_DATE_TIME]`
a|Time of committal of the item.

h|*1..1*
|*change_type*: `xref:{class_dv_coded_text}[DV_CODED_TEXT]`
a|Type of change. Coded using the openEHR Terminology  audit change type  group.

h|*0..1*
|*description*: `xref:{class_dv_text}[DV_TEXT]`
...
h|*1..1*
|*committer*: `xref:{class_party_proxy}[PARTY_PROXY]`
