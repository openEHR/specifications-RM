...
h|*1..1*
|*persist_in_server*: `xref:{class_boolean}[Boolean]`
a|If True, this Request is persisted in the server until further notice.

h|*0..1*
|*repeat_period*: `xref:{class_dv_duration}[DV_DURATION]`
a|Period for resending update Extracts in response to original Request.

h|*0..1*
|*trigger_events*: `xref:{class_list}[List]<xref:{class_dv_coded_text}[DV_CODED_TEXT]>`
...
h|*1..1*
|*update_method*: `xref:{class_code_phrase}[CODE_PHRASE]`
