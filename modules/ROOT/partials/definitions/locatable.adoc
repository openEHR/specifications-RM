...
h|*1..1*
|*name*: `xref:{class_dv_text}[DV_TEXT]`
...
h|*1..1*
|*archetype_node_id*: `xref:{class_string}[String]`
...
h|*0..1*
|*uid*: `xref:{class_uid_based_id}[UID_BASED_ID]`
a|Optional globally unique object identifier for root points of archetyped structures.

h|*0..1*
|*links*: `xref:{class_list}[List]<xref:{class_link}[LINK]>`
...
h|*0..1*
|*archetype_details*: `xref:{class_archetyped}[ARCHETYPED]`
a|Details of archetyping used on this node.

h|*0..1*
|*feeder_audit*: `xref:{class_feeder_audit}[FEEDER_AUDIT]`
