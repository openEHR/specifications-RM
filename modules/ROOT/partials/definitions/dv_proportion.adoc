...
h|*Inherit*
2+|`xref:{class_proportion_kind}[PROPORTION_KIND]`, `xref:{class_dv_amount}[DV_AMOUNT]`
...
h|*1..1*
|*numerator*: `xref:{class_real}[Real]`
a|Numerator of ratio

h|*1..1*
|*denominator*: `xref:{class_real}[Real]`
a|Denominator of ratio.

h|*1..1*
|*type*: `xref:{class_integer}[Integer]`
...
h|*0..1 +
(redefined)*
|*normal_range*: `xref:{class_dv_interval}[DV_INTERVAL]<xref:{class_dv_proportion}[DV_PROPORTION]>`
a|Optional normal range.

h|*0..1 +
(redefined)*
|*other_reference_ranges*: `xref:{class_list}[List]<xref:{class_reference_range}[REFERENCE_RANGE]<xref:{class_dv_proportion}[DV_PROPORTION]>>`
...
): `xref:{class_dv_proportion}[DV_PROPORTION]`
