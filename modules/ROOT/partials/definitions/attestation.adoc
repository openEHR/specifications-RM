...
h|*Inherit*
2+|`xref:{class_audit_details}[AUDIT_DETAILS]`
...
h|*0..1*
|*attested_view*: `xref:{class_dv_multimedia}[DV_MULTIMEDIA]`
a|Optional visual representation of content attested e.g. screen image.

h|*0..1*
|*proof*: `xref:{class_string}[String]`
a|Proof of attestation.

h|*0..1*
|*items*: `xref:{class_list}[List]<xref:{class_dv_ehr_uri}[DV_EHR_URI]>`
...
h|*1..1*
|*is_pending*: `xref:{class_boolean}[Boolean]`
